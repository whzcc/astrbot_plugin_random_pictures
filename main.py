...
@register("random-pictures", "whzc", "随机发一些图片", "1.1.0", "repo url")
...
async def random_pictures(self, event: AstrMessageEvent):
    ...
    if message_str.startswith("/抽明信片"):
        ...
        if info != '':
            introduction = f"Agrato 为你介绍 {i} / {num_png} 号明信片：\n"+f"{info}"
        else:
            introduction = f"这张明信片没有介绍词。（{i} / {num_png} 号）"

        url = await self.html_render(TMPL,
    {"items": [introduction],"footer_image": b64_img}) # 第二个参数是 Jinja2 的渲染数据
